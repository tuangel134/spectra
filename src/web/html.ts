...
function wireConfig(){
  ...
  on("cpAdd",async()=>{ await jpost("/api/provider/custom",{id:$("cpId").value.trim(),baseURL:$("cpUrl").value.trim(),apiKey:$("cpKey").value.trim(),model:$("cpModel").value.trim()}); cfgReload(); await loadCatalog(); });
